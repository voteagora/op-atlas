...
import { createCitizenAttestation, createEntityAttestation } from "@/lib/eas"
...
async function createPrivyUserWithAddress(address: string): Promise<UserWithAddresses> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to create Privy user: ${response.status} ${errorText}`)
        }
        ...
        console.error(`Failed to create Privy user for address ${checksumAddress}:`, error)
        throw error
    }
}
...
async function createMockOrganization(userId: string, index: number) {
    ...
    console.log(`✅ Created entity attestation for organization: ${organizationId}`)
    ...
    console.log(`✅ Created/updated organization ${organizationId} for user ${userId}`)
    return organization
}

// Function to create a mock project with proper EAS attestation
async function createMockProject(userId: string, index: number, organizationId?: string) {
    // Create entity attestation first to get a valid UID
    const user = await prisma.user.findUnique({
        where: { id: userId },
        select: { farcasterId: true }
...
async function createMockCitizens() {
    for (let i = 0; i < addressesToCheck.length; i++) {
        ...
        try {
            console.log(`\n-------------------------------`)
            console.log(`Processing address ${i + 1}/${addressesToCheck.length}: ${address}`)
            ...
            if (!user) {
                ...
                console.log(`✅ Created new user ${user.id} with Privy DID: ${user.privyDid}`)
            }
            ...
            if (existingCitizen && existingCitizen.attestationId) {
                console.log(`Attestation already exists for user ${user.id} with type ${citizenType}!`)
                continue
            }

            const primaryAddress = user.addresses.find((addr: UserAddress) => addr.primary)?.address || address

            console.log(`Creating attestation for user ${user.id} with address ${primaryAddress} and type ${citizenType}`)

            const attestationId = await createCitizenAttestation({
                to: primaryAddress,
                farcasterId: parseInt(user.farcasterId || "0"),
                selectionMethod: CITIZEN_ATTESTATION_CODE[citizenType as keyof typeof CITIZEN_ATTESTATION_CODE],
            ...
            console.log(`✅ Successfully created ${citizenType} citizen record for user ${user.id} with attestation: ${attestationId}`)
