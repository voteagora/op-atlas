...
export const SafeContextWrapper = ({
  ...
  const currentWalletDisplay = currentContext === 'SAFE' 
    ? `${selectedSafeWallet?.address.slice(0, 6)}...${selectedSafeWallet?.address.slice(-4)}`
        ...
        <div className="bg-card p-8 rounded-lg border shadow-lg max-w-md mx-4">
          ...
          <div className="space-y-4 text-sm text-muted-foreground">
            <p>
              {customMessage || (
                <>
                  <strong>{featureDisplayName}</strong> is not available when using Safe wallet context.
                </>
              )}
            </p>
