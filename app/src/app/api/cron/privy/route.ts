...
type LinkedAccount = EmailLinkedAccount | WalletLinkedAccount | FarcasterLinkedAccount;
                    ...
                    if (user.farcasterId) {
                        ...
                        if (farcasterAddresses.length > 0) {
                            try {
                                ...
                                if (isNaN(fidNumber)) {
                                    console.warn(`Invalid farcasterId for user ${user.id}: ${user.farcasterId}`);
                                } else {
                                ...
                                console.warn(`Error parsing farcasterId for user ${user.id}: ${e}`);
                            }
                        } else {
                            // Fallback to just using farcasterId without an address
                            console.warn(`User ${user.id} has farcasterId but no Farcaster addresses`);
                        }
                    }
