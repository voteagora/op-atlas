...
import { resolveSessionUserId, userOwnsAddress } from "@/lib/actions/utils"
  ...
  return withImpersonation(
    async ({ userId: sessionUserId, db }) => {
      ...
      try {
        ...
        const ownsGovernanceAddress = await userOwnsAddress(
          userId,
          checksummedAddress,
          db,
        )
        if (!ownsGovernanceAddress) {
          return { success: false, error: "Unauthorized" }
        }
        ...
        if (season.userCitizenLimit) {
          ...
          if (currentCount >= season.userCitizenLimit) {
            ...
            return {
              success: false,
              error:
                "Citizen registration limit has been reached for this season",
            }
          }
        }
